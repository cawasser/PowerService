...
The `home-routes` handler in the `power-service.routes.home` namespace
...
The `home-page` function will in turn call the `power-service.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `power-service.handler` namespace:
...
Request middleware functions are located under the `power-service.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `power-service.dev-middleware` namespace found in
